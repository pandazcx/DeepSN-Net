...
class newton(nn.Module):
    ...
    def forward(self, z):
        ...
        x, mid = self.initialization(dz)
        for i in range(self.depth - 1):
            x = self.image_update_list[i](x, dz, mid)
...
if __name__ == '__main__':
    ...
    img = image_read("C:\\Users\\China\\PycharmProjects\\FinalNet\\analysise\\image\\GOPR0372_07_00-000047-1.png")#.cuda()

    #net = net.to(device)
    t1 = time.perf_counter()
    #print(f"Current GPU memory usage: {torch.cuda.memory_allocated(device='cuda') / 1024 ** 3:.2f} GB")
