...
sys.path.append("..")
from utils import padding,inv_padding,window_partitionx,window_reversex,realblur_cal
...
class Runner:
        ...
        for epoch_idx in range(last_epoch):
            ...
            for input_data in batch_list:
                ...
                if (current_idx) % self.config["val"]["freq"] == 0:
                    self.test(test_loader_dict["Test100"], "Test100",256)
                    self.test(test_loader_dict["Rain100H"], "Rain100H",-1)
                    self.test(test_loader_dict["Rain100L"], "Rain100L", -1)
                    self.test(test_loader_dict["Test1200"], "Test1200", 256)
                    self.test(test_loader_dict["Test2800"], "Test2800", 256)
    ...
    def test_iter(self,input_data, windows):
        if windows < 0:
            ...
            if H < windows or W < windows:
                LQ, pad = padding(windows, input_data)
                _, _, Hr, Wr = LQ.shape
                LQ_re, batch_list = window_partitionx(LQ, windows)
                start_time = time.perf_counter()
                output = self.model(LQ_re.to(self.rank))
                end_time  = time.perf_counter()
                output = window_reversex(output, windows, Hr, Wr, batch_list)
                output = inv_padding(pad, output)
            else:
    ...
    def test(self,test_loader, testset, windows, save_img = False):
        loss_function = self.make_loss()
        if self.rank == 0:
        ...
        self.model.eval()
        tmp_results = {'LOSS':[],'Time':[]}
        with torch.no_grad():
            batch_list = tqdm(test_loader) if self.rank == 0 else test_loader
            for batch_idx, input_data in enumerate(batch_list, 0):
                output,cal_time = self.test_iter(input_data["LQ"], windows)

                loss = loss_function(output, input_data["HQ"].to(self.rank)).data.item()
                ...
                tmp_results['LOSS'].append(loss)

                if self.rank == 0:
                    batch_list.set_description("LOSS:%.2f" % loss)
