...
class Runner:
    ...
    def val(self,test_loader, save_img = False):
        if self.rank == 0:
            print("======= SIDD val ========")
            logging.info("======= SIDD val ========")

        if save_img and self.rank == 0:
            folder = os.path.join(self.save_path, "SIDD_val")
    ...
    def DND_benchmark_submit(self,DND_loader):
        if self.rank == 0:
            print("======= DND Test ========")
            logging.info("======= DND Test ========")
        folder = os.path.join(self.save_path, "DND_Test")
        os.makedirs(folder, exist_ok=True)

        self.model.eval()
        with torch.no_grad():
            batch_list = tqdm(DND_loader) if self.rank == 0 else DND_loader
            for batch_idx, input_data in enumerate(batch_list, 0):
                Idenoised = np.zeros((20,), dtype=object)
                boxes = input_data["boxes"][0]
                Inoisy = input_data["Inoisy"]
                filename = input_data["filename"][0]
                for k in range(20):
                    DND_idx = [int(boxes[k,0]-1),int(boxes[k,2]),int(boxes[k,1]-1),int(boxes[k,3])]
                    noisy_patch = Inoisy[:,DND_idx[0]:DND_idx[1],DND_idx[2]:DND_idx[3], :].permute(0,3,1,2).to(self.rank)
                    output,_ = self.test_iter(noisy_patch, 256)
                    Idenoised[k] = output.cpu().detach().permute(0, 2, 3, 1).squeeze(0).numpy()

                # if self.rank == 0:
                sio.savemat(os.path.join(folder, filename),
                            {"Idenoised": Idenoised,
                             "israw": False,
                             "eval_version": "1.0"},)
    ...
    def SIDD_benchmark_submit(self,SIDD_loader,block_len):
        if self.rank == 0:
            print("======= SIDD Test ========")
            logging.info("======= SIDD Test ========")
        folder = os.path.join(self.save_path, "SIDD_Test")
        os.makedirs(folder, exist_ok=True)
        self.model.eval()
        with torch.no_grad():
            batch_list = tqdm(SIDD_loader) if self.rank == 0 else SIDD_loader
            poseSmile_cell = np.empty((len(SIDD_loader), block_len), dtype=object)
            for batch_idx, input_data in enumerate(batch_list, 0):
                noisy_image = input_data["noisy_image"]
                for block_index in range(block_len):
                    noisy_patch = noisy_image[:,block_index,:,:,:]
                    output,_ = self.test_iter(noisy_patch, -1)
                    output = np.transpose(output.cpu().numpy().squeeze(), (1, 2, 0))
                    output = img_as_ubyte(output)
                    poseSmile_cell[batch_idx, block_index] = output
            submit_data = {'DenoisedBlocksSrgb': poseSmile_cell}
            savemat(
                os.path.join(folder, 'SubmitSrgb.mat'),
                submit_data
            )
